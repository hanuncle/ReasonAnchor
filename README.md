...
The platform packages specific capabilities as pluggable modules. Each module can carry code, workflows, configuration fields, knowledge assets, Skill instructions, a machine-readable playbook, raw-output sorters, and a final-result schema. MCP-capable AI clients, such as Codex, Claude Desktop, Cursor, or other compatible agent hosts, can list modules, load only the selected module Skill, run module functions, save final results, and, after user approval, iterate module code so future runs use fewer tokens and produce better output.
...
- **Pluggable modules**: create, import, export, package, and validate modules.
...
- `http://127.0.0.1:8111/`: upload samples, view modules, choose functions, run workflows, edit local config.
- `http://127.0.0.1:8111/raw-output.html?session_id=<session_id>`: inspect preserved raw function output.
- `http://127.0.0.1:8111/result.html?session_id=<session_id>`: view the final AI summary saved for a session.
